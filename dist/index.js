...
function securityRules(profile) {
    return profile.securityRules.map(rule => `- ${rule}`).join("\n");
}
function autonomyRules(profile) {
...
function projectContext(profile) {
...
function claudeContext(profile) {
    return `${projectContext(profile)}
...
const SECURITY_RULES = [
    {
        id: "hardcoded-secret",
        severity: "high",
        pattern: /(?:api[_-]?key|secret|password|passwd|passphrase|access[_-]?token|auth[_-]?token|private[_-]?key)\s*(?:=|:|=>)\s*["'][^"'\s]{8,}["']/i,
    ...
    },
    {
        ...
        severity: "high",
        pattern: /(?:SELECT|INSERT INTO|UPDATE|DELETE FROM)\b[^;\n]*(?:\$\{|\$_(?:GET|POST|REQUEST|COOKIE)|"\s*\+\s*|'\s*\.\s*\$)/i,
    ...
    },
    {
        id: "shell-execution",
        severity: "high",
        pattern: /(?:shell_exec|passthru|popen|proc_open|system)\s*\(|\bexec\s*\(\s*[`"'][^`"']*(?:\$\{|\$_)|child_process[\s\S]{0,40}\bexec\s*\(/,
    ...
    },
    {
        ...
        severity: "high",
        pattern: /rejectUnauthorized\s*:\s*false|CURLOPT_SSL_VERIFY(?:PEER|HOST)\s*(?:,|=>)\s*(?:false|0)|verify\s*=\s*False/,
        message: "TLS certificate verification disabled."
    },
    {
        id: "path-traversal",
        severity: "medium",
        pattern: /(?:file_get_contents|fopen|readFile(?:Sync)?|include|require)\s*\(?\s*[^;\n]{0,40}(?:\$_(?:GET|POST|REQUEST)|req\.(?:query|params|body))/,
...
function collectFiles(directory, collected) {
    ...
    for (const entry of entries) {
        ...
        if (!isEnvFile && !SCAN_EXTENSIONS.has(extension))
            continue;
        try {
...
function securityReport(profile, findings) {
    ...
    const sections = order
        ...
        if (!group.length)
            return `${heading}\n\nNone detected.`;
        const rows = group
            .map(finding => `- \`${finding.file}:${finding.line}\` **${finding.rule}** — ${messages.get(finding.rule) ?? ""}\n  \`\`\`\n  ${finding.excerpt}\n  \`\`\``)
            .join("\n");
        return `${heading}\n\n${rows}`;
    })
        .join("\n\n");
...
const AI_TOOLS = ["devin", "claude", "cursor"];
function parseTool(value) {
    if (!value)
        return undefined;
    if (value === "all")
        return AI_TOOLS;
    return AI_TOOLS.includes(value) ? [value] : undefined;
}
async function selectTools() {
    ...
    if (selectedFromArgument)
        return selectedFromArgument;
    ...
    try {
        ...
        if (!selected)
            throw new Error("Invalid selection. Choose 1, 2, 3, or 4.");
        return selected;
    }
    finally {
        prompt.close();
...
function writeOutput(profile, selectedTools) {
    ...
    fs.writeFileSync(path.join(outputDir, "commands.md"), `# Detected Commands\n\n${commands(profile)}\n`);
...
switch (command) {
    ...
    case "security": {
        ...
        const outputDir = path.join(root, ".ai");
        fs.mkdirSync(outputDir, { recursive: true });
        fs.writeFileSync(path.join(outputDir, "security-report.md"), securityReport(detect(), findings));
